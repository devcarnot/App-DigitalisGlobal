        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5 [scrollbar-color:rgba(100,116,139,0.35)_transparent] [scrollbar-width:thin]">
          {loading && !url ? (
            ...
            <img
              src={url}
              alt={name}
              className="mx-auto max-h-[min(70vh,640px)] w-auto rounded-2xl border border-slate-200 bg-white shadow-sm"
            />
          ) : isVideo(path, mime) ? (
            <video
              src={url}
              controls
              className="mx-auto max-h-[min(70vh,640px)] w-full rounded-2xl border border-slate-200 bg-black shadow-sm"
            />
          ) : isAudio(path, mime) ? (
            ...
            <iframe
              title={name}
              src={url}
              className="mx-auto block h-[min(78vh,720px)] w-full rounded-2xl border border-slate-200 bg-slate-100 shadow-sm"
            />
          ) : isOffice(path, mime) ? (
            ...
            textError ? (
              ...
              <pre className="max-h-[min(70vh,640px)] overflow-auto whitespace-pre-wrap break-words rounded-2xl border border-slate-200 bg-slate-50 p-4 text-[12px] leading-relaxed text-slate-800">
                {textContent}
              </pre>
            )
          ) : (
