...
function statusPillClass(s) {
  if (s === 'approved') return 'bg-sky-100 text-sky-900 ring-sky-200/80';
  ...
  return (
    <div className="w-full space-y-6 text-[13px] leading-snug text-slate-800">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="relative overflow-hidden rounded-2xl border border-sky-200/55 bg-gradient-to-br from-sky-50/90 via-white to-white p-5 shadow-[0_12px_36px_-20px_rgba(14,116,144,0.18)] ring-1 ring-sky-900/[0.04]">
          <div className="pointer-events-none absolute -right-8 -top-10 h-28 w-28 rounded-full bg-sky-400/20 blur-2xl" aria-hidden />
          <p className="text-[11px] font-semibold text-sky-900/85">Approved ({year})</p>
          <p className="mt-2 text-2xl font-bold tabular-nums text-slate-900">{stats.approved}</p>
          <p className="mt-1 text-[12px] text-slate-600">Calendar days · working from home</p>
        </div>
        <div className="relative overflow-hidden rounded-2xl border border-amber-200/55 bg-gradient-to-br from-amber-50/80 via-white to-white p-5 shadow-[0_12px_36px_-20px_rgba(180,83,9,0.12)] ring-1 ring-amber-900/[0.05]">
          <div className="pointer-events-none absolute -right-8 -top-10 h-28 w-28 rounded-full bg-amber-400/15 blur-2xl" aria-hidden />
          <p className="text-[11px] font-semibold text-amber-900/80">Pending ({year})</p>
          <p className="mt-2 text-2xl font-bold tabular-nums text-slate-900">{stats.pending}</p>
          <p className="mt-1 text-[12px] text-slate-600">Days awaiting approval</p>
        </div>
      </div>

      <form
        onSubmit={(e) => void onSubmit(e)}
        className="space-y-4 rounded-2xl border border-sky-200/45 bg-white/95 p-5 shadow-[0_14px_40px_-22px_rgba(14,116,144,0.14)] ring-1 ring-sky-900/[0.04] sm:p-6"
      >
        <h2 className="text-base font-bold text-[#103D4D]">New remote work request</h2>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-sky-200/70 bg-white px-3 py-2 text-sm text-slate-900 focus:border-[#103D4D]/40 focus:outline-none focus:ring-4 focus:ring-sky-400/15"
            />
          </div>
          <div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={2}
            className="w-full resize-y rounded-xl border border-sky-200/70 bg-white px-3 py-2 text-sm text-slate-900 focus:border-[#103D4D]/40 focus:outline-none focus:ring-4 focus:ring-sky-400/15"
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-sm sm:p-5">
        ...
        {loading ? (
          <div className="flex justify-center py-10">
            <div className="h-8 w-8 animate-spin rounded-full border-[3px] border-sky-200 border-t-[#103D4D] border-r-violet-500" />
          </div>
        ) : rows.length === 0 ? (
              ...
              >
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-semibold text-slate-900">Remote work</span>
