...
function cardTone(key) {
  if (key === 'open') return 'ring-slate-400/30 bg-gradient-to-b from-slate-100/95 to-slate-50/85 border border-slate-300/35';
  if (key === 'in_progress') return 'ring-sky-400/35 bg-gradient-to-b from-sky-100/90 to-cyan-50/75 border border-sky-300/40';
  if (key === 'in_review') return 'ring-violet-400/35 bg-gradient-to-b from-violet-100/85 to-fuchsia-50/55 border border-violet-300/40';
  if (key === 'done') return 'ring-emerald-400/30 bg-gradient-to-b from-emerald-100/90 to-teal-50/70 border border-emerald-300/40';
  return 'ring-rose-400/30 bg-gradient-to-b from-rose-100/85 to-red-50/60 border border-rose-300/35';
}

/** Dark gradient strip per task-status column */
function columnHeaderClass(key) {
  if (key === 'open') return 'bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 text-white shadow-md shadow-slate-900/20';
  if (key === 'in_progress') return 'bg-gradient-to-r from-sky-900 via-cyan-800 to-teal-900 text-white shadow-md shadow-cyan-900/25';
  if (key === 'in_review') return 'bg-gradient-to-r from-violet-900 via-indigo-900 to-violet-950 text-white shadow-md shadow-violet-900/25';
  if (key === 'done') return 'bg-gradient-to-r from-emerald-950 via-teal-900 to-emerald-900 text-emerald-50 shadow-md shadow-emerald-900/20';
...
export default function MyTasksBoard({ embedded = false, standalonePage = false }) {
  ...
  const shell =
    embedded
      ...
      : 'rounded-2xl border border-cyan-200/45 bg-white/80 backdrop-blur-md overflow-hidden shadow-[0_12px_40px_-12px_rgba(16,61,77,0.18)] ring-1 ring-white/70';
  const innerPad = embedded ? 'p-2 sm:p-2.5' : 'p-5 sm:p-6';
  const titleClass = embedded
    ? 'text-xs font-bold uppercase tracking-wide text-teal-900/75'
    : standalonePage
      ? 'text-xl font-bold tracking-tight bg-gradient-to-r from-slate-900 via-[#103D4D] to-teal-600 bg-clip-text text-transparent'
      : 'text-lg font-bold tracking-tight bg-gradient-to-r from-slate-800 to-[#103D4D] bg-clip-text text-transparent';
      ...
      <>
        <div className={shell}>
          <div className={`${innerPad} flex items-center gap-3`}>
            ...
            <p className="text-teal-900/70 text-sm font-medium">Loading…</p>
          </div>
        </div>
        {myTasksModals}
      </>
      ...
      <>
        <div className={shell}>
          <div className={innerPad}>
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  {canCreateProject ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-full border border-violet-300/80 bg-gradient-to-r from-violet-700 to-indigo-800 px-3.5 py-2 text-[11px] font-bold text-white shadow-lg shadow-violet-900/25 transition hover:from-violet-800 hover:to-indigo-900"
                    >
            ...
            <div className="rounded-2xl border border-dashed border-cyan-300/60 bg-gradient-to-br from-cyan-50/40 via-white to-violet-50/30 px-6 py-14 text-center shadow-inner ring-1 ring-cyan-900/[0.04]">
    ...
    <div className={shell}>
      <div className={innerPad}>
          ...
          <div className="flex w-full min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
            <div className="flex w-full min-w-0 flex-col gap-2 sm:w-auto sm:flex-1 sm:flex-row sm:items-center sm:justify-end">
              ...
              {projectIds.length > 0 && isWorkspaceAdmin ? (
                <div
                  className="flex shrink-0 rounded-xl border border-cyan-200/70 bg-gradient-to-r from-slate-800/90 to-slate-900 p-0.5 shadow-md shadow-slate-900/15"
                  role="tablist"
                ...
                >
                  <button
                    ...
                    className={`rounded-lg px-3 py-1.5 text-[11px] font-bold transition-all ${
                      taskScope === 'mine'
                        ? 'bg-gradient-to-r from-cyan-400 to-teal-500 text-slate-900 shadow-md'
            ...
            {!embedded && (
              <>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full border border-cyan-300/80 bg-gradient-to-r from-[#103D4D] to-teal-700 px-3.5 py-2 text-[11px] font-bold text-white shadow-lg shadow-teal-900/20 transition hover:from-[#0d3442] hover:to-teal-800 disabled:opacity-50 disabled:pointer-events-none"
                >
                  <span aria-hidden>+</span>
                  Add task
                </button>
                <div
                  className="flex rounded-xl border border-cyan-200/70 bg-gradient-to-r from-slate-800/90 to-slate-900 p-0.5 shrink-0 shadow-md shadow-slate-900/15"
                  role="tablist"
        ...
        {projectIds.length > 0 && boardSearch.trim() && visibleProjectIds.length === 0 ? (
          <p className="mb-4 rounded-xl border border-amber-200/80 bg-amber-50/90 px-3 py-2 text-xs font-medium text-amber-950">
          ...
          {COLUMNS.map((col) => {
            ...
            return (
              <div
                ...
                } ${isDropOver ? 'ring-2 ring-cyan-500 ring-offset-2 ring-offset-cyan-50 shadow-lg shadow-cyan-900/15' : ''}`}
                onDragOver={
                  canDragKanban
                    ? (e) => {
                        e.preventDefault();
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          className={`overflow-hidden rounded-xl border border-cyan-200/50 bg-white/95 backdrop-blur-sm shadow-md shadow-cyan-900/8 transition-all hover:shadow-lg hover:border-cyan-400/50 hover:ring-1 hover:ring-violet-200/40 ${embedded ? 'text-[11px]' : ''}`}
                        >
                          <Link
                          ...
                          >
                            <div className="flex items-start justify-between gap-1.5">
                              <p
                                className={`font-bold text-slate-900 leading-snug break-words min-w-0 line-clamp-3 ${embedded ? 'text-xs' : 'text-sm'}`}
                              >
                            ...
                            <p
                              className={`mt-1.5 line-clamp-1 text-slate-500 leading-snug ${embedded ? 'text-[10px]' : 'text-[11px]'}`}
                              title={projectName}
                            >
          ...
          {COLUMNS.map((col) => {
            ...
            return (
              ...
              >
                ...
                <ul className="divide-y divide-cyan-100/60 bg-white/90 backdrop-blur-sm">
                  {tasks.map((task) => {
                    const pid = task.project_id;
                    ...
                    return (
                      <li
                        key={task.id}
                        className="px-3 py-2.5 hover:bg-gradient-to-r hover:from-cyan-50/50 hover:to-violet-50/30 transition-colors"
                      >
                        <div
                          onContextMenu={
                            isWorkspaceAdmin
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            >
                              <div className="flex items-start justify-between gap-2">
                                <p
                                  className={`font-semibold text-slate-900 leading-tight break-words min-w-0 ${embedded ? 'text-xs' : 'text-sm'}`}
                                >
                                  {task.title || 'Untitled task'}
                                </p>
                                <ReadOnlyPriorityPill priority={normalizeTaskPriority(task.priority)} />
                              ...
                              <p
                                className={`mt-1 line-clamp-1 text-slate-600 ${embedded ? 'text-[10px]' : 'text-[11px]'}`}
                                title={projectName}
                              >
                                <span className="font-medium text-slate-400">Project</span>{' '}
                                <span className="font-semibold text-slate-700">{projectName}</span>
                  ...
                  {tasks.length === 0 && (
                    <li className="py-8 text-center text-xs text-teal-800/45 font-medium">No tasks in this stage</li>
                  )}
                </ul>
              </section>
            );
