-- Allow Super Admin / Team Manager to undo an accidental check-out
-- (clears check_out_at so the member can check out again later).
...
COMMENT ON FUNCTION public.erp_attendance_admin_undo_checkout_pk(uuid) IS
  'Super Admin or Team Manager (shared project peers only) clears check_out_at after an accidental checkout.';

GRANT EXECUTE ON FUNCTION public.erp_attendance_admin_undo_checkout_pk(uuid) TO authenticated;
