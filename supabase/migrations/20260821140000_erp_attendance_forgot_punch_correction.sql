-- Allow members marked absent to request full-day check-in/out correction (forgot to punch).
...
ALTER TABLE public.erp_attendance_correction_requests
  ...
  CHECK (kind IN ('missing_checkout', 'absent_explain', 'forgot_punch'));
...
BEGIN
  ...
  IF p_kind NOT IN ('missing_checkout', 'absent_explain', 'forgot_punch') THEN
